...
// ao_star is the main algorithm. It uses a priority queue to keep track of states ordered by their estimated cost. We maintain a map of g_values to store the cost to reach each state.

// In the main function, we take user input for grid dimensions and the coordinates of the monkey, banana, and box.

// We initialize the start and goal states and call ao_star to find the solution.

// Finally, we print the result. If a valid path is found, we display the number of moves required; otherwise, we indicate that no path exists.
